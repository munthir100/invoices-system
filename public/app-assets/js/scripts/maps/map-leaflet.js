...
$(function () {
  ...
  if ($('#customerLocation').length) {
    var draggableMap = L.map('customerLocation').setView([48.817152, 2.455], 12);
    var markerLocation = L.marker([48.817152, 2.455], {
      draggable: true
    }).addTo(draggableMap);
    markerLocation.bindPopup("<b>Drag Customer Location!</b>").openPopup();

    // Add a moveend event listener to the marker
    markerLocation.on('dragend', function (event) {
      var latlng = event.target.getLatLng();

      // Set the values of the input fields with the updated coordinates
      $('#lat').val(latlng.lat);
      $('#lang').val(latlng.lng);
    });
  ...
  if ($('#editCustomerLocation').length) {
    var lang = $('#lang').val();
    var lat = $('#lat').val();

    var draggableMap = L.map('editCustomerLocation').setView([lat, lang], 12);
    var markerLocation = L.marker([lat, lang], {
      draggable: true
    }).addTo(draggableMap);
    markerLocation.bindPopup("<b>Current Customer Location!</b>").openPopup();
